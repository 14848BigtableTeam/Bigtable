...
def post_sharding(host, port, table_name, midle_row):
    ...
    for i in range(len(tablet)):
        sharding_tablet_number = (tablet_num + i) % len(tablet) + 1
        sharding_tablet = "tablet" + str(sharding_tablet_number)
        if sharding_tablet not in tablet_list:
            metadata[tablet_name]["tablet"].append({"hostname": tablet[sharding_tablet]["host"], "port": str(tablet[sharding_tablet]["port"]), "row_from": midle_row,
         "row_to": row_to})
